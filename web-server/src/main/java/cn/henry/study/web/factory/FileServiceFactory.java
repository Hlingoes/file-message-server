package cn.henry.study.web.factory;

import cn.henry.study.web.constants.HeaderConstants;
import cn.henry.study.web.entity.MessageBrief;
import cn.henry.study.web.service.files.DefaultFileService;
...
public class FileServiceFactory {
    ...
    public void event(ApplicationReadyEvent event) {
        ...
        map.forEach((key, value) -> {
            if (null != value.getEntityClazz()) {
                ...
                File preFailLog = FileUtils.getFile(FileHelpUtils.findHomeDir(FileServiceFactory.class), this.siftLogHome, logPath);
                if (preFailLog.exists()) {
                    this.cacheFailData(preFailLog);
                }
            }
        });
     ...
     */
    private void cacheFailData(File activeFile) {
        try {
    ...
    public void reUploadFiles() {
        ...
        this.failFilesCacheMap.forEach((key, value) -> {
            ...
            list.forEach(brief -> {
                ...
                if (brief.getRetryFile().exists()) {
                    this.serviceCacheMap.get(className).upload(brief.getRowKey(), brief.getRetryFile());
                    brief.getRetryFile().delete();
                    logger.info("上传完成: {}, 删除文件: {}", brief, brief.getRetryFile().getAbsolutePath());
                }
            });
        });
    }
